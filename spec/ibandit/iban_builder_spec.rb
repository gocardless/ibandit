...
describe Ibandit::IBANBuilder do
  ...
  describe '.build' do
    ...
    context 'with an unsupported country_code' do
      ...
      its([:iban]) { is_expected.to be_nil }
      its([:country_code]) { is_expected.to eq('FU') }
    end
    ...
    context 'with AT as the country_code' do
      ...
      its([:country_code]) { is_expected.to eq('AT') }
      its([:bank_code]) { is_expected.to eq('19043') }
    ...
    context 'with CY as the country_code' do
      ...
      its([:country_code]) { is_expected.to eq('CY') }
      ...
      context 'with the branch code inside the bank_code' do
    ...
    context 'with DE as the country_code' do
      ...
      its([:bank_code]) { is_expected.to eq('37040044') }
      its([:account_number]) { is_expected.to eq('0532013000') }
      ...
      context 'with a pseudo account number' do
        ...
        its([:bank_code]) { is_expected.to eq('37080040') }
      end
    end
    ...
    context 'with EE as the country_code' do
      ...
      its([:country_code]) { is_expected.to eq('EE') }
    ...
    context 'with FI as the country_code' do
      ...
      its([:country_code]) { is_expected.to eq('FI') }
      its([:bank_code]) { is_expected.to eq('123456') }
      ...
      context 'with a savings bank account_number in traditional format' do
        ...
        its([:bank_code]) { is_expected.to eq('423456') }
    ...
    context 'with FR as the country_code' do
      ...
      context 'without the rib key in the account number' do
        ...
        its([:account_number]) { is_expected.to eq('0500013M026') }
      end
    ...
    context 'with GB as the country_code' do
      ...
      context 'with both a bank_code and a bic_finder' do
        ...
        it 'uses the explicitly provided bank code' do
          expect(subject[:iban]).to eq('GB07BARC20000000579135')
          expect(subject[:bank_code]).to eq('BARC')
        end
      end
    end
    ...
    context 'with IE as the country_code' do
      ...
      its([:country_code]) { is_expected.to eq('IE') }
      ...
      context 'with both a bank_code and a bic_finder' do
        ...
        it 'uses the explicitly provided bank_code' do
          ...
          expect(subject[:bank_code]).to eq('AIBK')
        end
      end
    end
    ...
    context 'with IT as the country_code' do
      ...
      its([:country_code]) { is_expected.to eq('IT') }
      its([:bank_code]) { is_expected.to eq('05428') }
      its([:branch_code]) { is_expected.to eq('11101') }
    ...
    context 'with LT as the country_code' do
      ...
      its([:country_code]) { is_expected.to eq('LT') }
      its([:account_number]) { is_expected.to eq('11101001000') }
      its([:bank_code]) { is_expected.to eq('10000') }
    ...
    context 'with LU as the country_code' do
      ...
      its([:country_code]) { is_expected.to eq('LU') }
      its([:account_number]) { is_expected.to eq('9400644750000') }
      its([:bank_code]) { is_expected.to eq('001') }
    ...
    context 'with LV as the country_code' do
      ...
      its([:country_code]) { is_expected.to eq('LV') }
      its([:account_number]) { is_expected.to eq('1234567890123') }
      its([:bank_code]) { is_expected.to eq('BANK') }
    ...
    context 'with MC as the country_code' do
      ...
      its([:bank_code]) { is_expected.to eq('20041') }
      its([:branch_code]) { is_expected.to eq('01005') }
    ...
    context 'with NL as the country_code' do
      ...
      its([:country_code]) { is_expected.to eq('NL') }
      its([:account_number]) { is_expected.to eq('0417164300') }
      its([:bank_code]) { is_expected.to eq('ABNA') }
    ...
    context 'with PT as the country_code' do
      ...
      its([:country_code]) { is_expected.to eq('PT') }
      its([:bank_code]) { is_expected.to eq('0002') }
      its([:branch_code]) { is_expected.to eq('0023') }
      its([:account_number]) { is_expected.to eq('0023843000578') }
    ...
    context 'with SK as the country_code' do
      ...
      its([:country_code]) { is_expected.to eq('SK') }
      its([:bank_code]) { is_expected.to eq('1200') }
    ...
    context 'with SM as the country_code' do
      ...
      its([:country_code]) { is_expected.to eq('SM') }
      its([:bank_code]) { is_expected.to eq('05428') }
      its([:branch_code]) { is_expected.to eq('11101') }
      its([:account_number]) { is_expected.to eq('000000123456') }
