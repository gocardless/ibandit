...
IBAN Bru is a Ruby library for manipulating and validating
...
```ruby
gem install iban-bru
```

### Creating IBANs

All functionality is based around `IBAN` objects. To create one, simply pass a
string to `IbanBru::IBAN.new`. Note that you don't need to worry about case and
whitespace:

```ruby
iban = IbanBru::IBAN.new("xq75 B a dCode 666")
...
```ruby
iban = IbanBru::IBAN.new("XQ75 BADCODE 666")
...
```ruby
iban = IbanBru::IBAN.new("GB82 WEST 1234 5698 7654 32")
...
```ruby
# Austria
iban = IbanBru::IBANBuilder.build(
  country_code: 'AT',
